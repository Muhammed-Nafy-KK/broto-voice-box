...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const complaintChannel = supabase.channel('complaint-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'complaints',
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Bro to Raise</h1>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        {complaints.length === 0 ? <div className="text-center py-12">
            ...
            {complaints.map(complaint => <ComplaintCard key={complaint.id} complaint={complaint} onClick={() => navigate(`/student/complaint/${complaint.id}`)} />)}
          </div>}
      </main>
    </div>;
};
